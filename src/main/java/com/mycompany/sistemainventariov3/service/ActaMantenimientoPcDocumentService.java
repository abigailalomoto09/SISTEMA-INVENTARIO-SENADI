...
public class ActaMantenimientoPcDocumentService {
    ...
    // PDF row heights (pt) — sized so ALL sections fit on one A4 page
    ...
    private static final float H_ACTL  = 30f;  // long text activity
    private static final float H_ACTO  = 40f;  // "otras acciones"
    private static final float H_SIGN  = 65f;  // firma space
    private static final float FOOTER_Y = 78f; // reserved bottom area

    // DOCX row heights (twips = 1/20 pt)
    ...
    public byte[] generarPdf(ActaMantenimientoPcRequest request) throws IOException {
        ...
        try (PDDocument doc = new PDDocument();
            ...
            doc.addPage(page);
            float pw  = PDRectangle.A4.getWidth();   // 595.28
            float ph  = PDRectangle.A4.getHeight();  // 841.89
            float mg  = 40f;
            float cw  = pw - 2 * mg;                // 515.28

            try (PDPageContentStream cs = new PDPageContentStream(doc, page)) {
                // ── Header logos at fixed positions ──────────────────────────
                float logoY = ph - 28f;
                ...
                // ── Content sections (flow from top, must stay above FOOTER_Y) ──
                ...
                y = drawParagraph(cs, normalize(request.getCertificacion()),
                                  mg + 6f, y, cw - 12f, 8.5f, 11f, 5);
                y -= 8f;
                drawEntregaRecepcionSection(cs, mg, y, cw, request);

                // ── Footer at absolute bottom — never overlaps content ────────
                drawHorizontalLine(cs, mg, FOOTER_Y - 2f, cw, 0.5f);
                drawFooterText(cs, mg, FOOTER_Y - 12f);
                drawImage(cs, doc, "actas/logo_nuevo_ecuador.png",
                          pw - mg - 128f, 32f, 128f, 46f);
            }
            doc.save(out);
            return out.toByteArray();
        }
    ...
    private void addEquiposTable(XWPFDocument doc, ActaMantenimientoPcRequest req) {
        ...
        XWPFTable t = createStyledTable(doc, 4, 5, grid);
        setRowHeight(t.getRow(0), DX_LBL);
        setRowHeight(t.getRow(1), DX_LBL);
        setRowHeight(t.getRow(2), DX_DATA);
        setRowHeight(t.getRow(3), DX_DATA);
        mergeCellsH(t, 0, 0, 4);
        ...
        fillEquipoRow(t.getRow(3), req.getLaptop());
    }

    private void addActividadesTable(XWPFDocument doc, ActaMantenimientoPcRequest req) {
        int[] grid = {4200, 1700, 1400, 3166};
        XWPFTable t = createStyledTable(doc, 10, 4, grid);
        setRowHeight(t.getRow(0), 480);
        setRowHeight(t.getRow(1), 360);
        setRowHeight(t.getRow(2), 360);
        for (int i = 3; i < 10; i++) setRowHeight(t.getRow(i), 440);

        mergeCellsH(t, 0, 0, 3);
        setCellText(t.getRow(0).getCell(0), "COMPUTADORA", true, ParagraphAlignment.CENTER, "7F7F7F", "FFFFFF", 14);
    ...
    /** Footer as content table — compact, no excess spacing. */
    ...
    private float drawEquiposSection(PDPageContentStream cs, float x, float y, float w,
        ...
        y = drawRow(cs, x, y, cols,
                new String[]{req.getLaptop().getTipo(), req.getLaptop().getMarca(),
                             req.getLaptop().getModelo(), req.getLaptop().getSerial(),
                             req.getLaptop().getCodigo()}, H_ROW, false, Color.WHITE);
        return y;
    }

    private float drawActividadesSection(PDPageContentStream cs, float x, float y, float w,
                                         ActaMantenimientoPcRequest req) throws IOException {
        float[] actH = {H_ACT, H_ACT, H_ACT, H_ACTL, H_ACT, H_ACT, H_ACTO};
        ...
        drawText(cs, "COMPUTADORA", x + w/2f, y - H_DARK*0.65f, PDType1Font.HELVETICA_BOLD, 12f, Color.WHITE, true);
        float cur = y - H_DARK;

        // Sub-header 1: activity label + INSTALADO span
    ...
    /**
     * Create a table with borders, table-fixed layout, and exact column grid.
     * This is the single safe entry point for all content tables.
     */
    ...
    /**
     * Replace any existing tblGrid and install exact column widths (twips).
     * Uses XmlBeans selectChildren to remove duplicates before adding new grid.
     */
    private void applyTableGrid(XWPFTable table, int... colWidths) {
        ...
        // Remove all existing tblGrid elements (avoids the duplicate-grid corruption)
    ...
    /**
     * Set table layout to FIXED so Word does not auto-resize columns.
     * Uses XmlBeans cursor to insert the tblLayout element into tblPr.
     */
    ...
    private void ensureActaDefaults(ActaMantenimientoPcRequest req) {
        ...
        if (req.getLaptop()      == null) req.setLaptop(new ActaMantenimientoPcRequest.EquipoRow());
        ...
        if (req.getDesktop().getTipo() == null || req.getDesktop().getTipo().trim().isEmpty())
            req.getDesktop().setTipo("DESKTOP");
        if (req.getLaptop().getTipo()  == null || req.getLaptop().getTipo().trim().isEmpty())
            req.getLaptop().setTipo("LAPTOP");
    }
